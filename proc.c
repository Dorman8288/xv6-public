...
clone(void(*fcn)(void*,void*), void *arg1, void *arg2, void* stack)
{
  ...
  np->parent = curproc;
  *np->tf = *curproc->tf;
  curproc->count_thread=curproc->count_thread+1;
  np->is_thread=1;
  ...
  *(uint*)sarg2 = (uint)arg2;
  ...
  np->tf->esp = (uint)np->threadstack + PGSIZE - 3 * sizeof(void*);
  np->tf->ebp = np->tf->esp;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if((p->state != RUNNABLE && p->count_thread==0)|| p->is_thread==1)
        continue;
      ...
      for(p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++){
        if(p1->state != RUNNABLE|| p1->is_thread==0|| p1->parent!=p)
          continue;
        count_threads++; 
      }
      if(p->turn>count_threads){
        p->turn=0;
      }
      if(p->count_thread>0){
        if(p->turn==0){
        ...
        p->turn=(p->turn+1)%(count_threads+1);
      }
      else{
        c->proc = p;
        switchuvm(p);
  ...
  //   int ranges[NPROC + 1];
  //   double sum = 0;
  //   struct proc* prevProccess=ptable.proc;
  //   int numProccess = 0;
  //   for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  ...
  //   struct proc *choosenProccess=ptable.proc;
  //   for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  //   {
  //     if(p->state != RUNNABLE)
  //       continue;
  ...
  //   }
  //   if(numProccess>=1){
  //     cprintf("%s\n",choosenProccess->name);
  //   }
  //   // if(choosenProccess->pid!=1)
  //   //   cprintf("prosess %d\n",choosenProccess->pid);
  //   for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
  //     if(p->state != RUNNABLE)
  //       continue;
